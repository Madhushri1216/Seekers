  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      for (var i = 0; i < response.length; i++) {
        ...
        $("#tableData").append('<tr><td>' + j + '</td><td style="display: none;">' + response[i].cd_id + '</td><td>' + response[i].cd_name + '</td><td>' + response[i].cd_fees + '</td><td>' + response[i].cd_total_seats_gov + '</td><td>' + response[i].cd_available_seats_gov + '</td><td>' + response[i].cd_total_seats_mng + '</td><td>' + response[i].cd_available_seats_mng + '</td><td><div class="d-flex" style="justify-content: space-evenly;"><a href="javascript:void(0);" id="edit_row" title="View/Edit" data-bs-toggle="modal" data-bs-target="#edit_modal"  class="text-primary" onClick="getRowsUpdate();">Edit</a><a href="javascript:void(0);" title="Delete" data-bs-toggle="modal" data-bs-target="#delete_modal" class="text-danger" id="delete_row" onClick="getRowsDelete();">Delete</a></div></td></tr>');
      }
    },
    error: function (request, error) {
      console.error(error);
