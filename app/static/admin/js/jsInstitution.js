$("#btn_add").click(function() {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        for (let i = 0; i < response.length; i++) {
          ...
          $("#tableData").append('<tr><td>' + j + '</td><td style="display: none;">' + response[i].in_id + '</td><td>' + response[i].in_name + '</td><td>' + response[i].in_mobile + '</td><td>' + response[i].in_email + '</td><td>' + response[i].in_address + '</td><td>' + response[i].in_about + '</td><td><div class="d-flex" style="justify-content: space-evenly;"><a href="javascript:void(0);" id="edit_row" title="View/Edit" data-bs-toggle="modal" data-bs-target="#edit_modal"  class="text-primary" onClick="getRowsUpdate();">Edit</a><a href="javascript:void(0);" title="Delete" data-bs-toggle="modal" data-bs-target="#delete_modal" class="text-danger" id="delete_row" onClick="getRowsDelete();">Delete</a></div></td></tr>');
        }
      },
      error: function (request, error) {
        console.error(error);
  ...
  function getRowsUpdate() {
    ...
    //   var lclUniversity = currentRow.find("td:eq(2)").text();
      ...
      var lclAbout = currentRow.find("td:eq(6)").text();
      // alert(lclUniversity);  
